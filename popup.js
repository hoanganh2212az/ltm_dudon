...
function callDoodle(classEle, api) {
  fetch(api)
    ...
    .then(function (doodle) {
      ...
      slider.addEventListener("input", function () {
        const value = parseInt(slider.value) / 100; // Convert range slider value to a scale factor (0-1)
        const doodleImg = document.querySelector(".ele-preview");
        doodleImg.style.transform = `scale(${value})`; // Use transform to scale image
        console.log(`Image scaled to: ${value}`);
    });
